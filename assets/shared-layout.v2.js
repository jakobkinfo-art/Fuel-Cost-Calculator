(() => {
  ...
  const mark = `
    <img src="/assets/images/fuel-cost-calculator-logo.v2.webp" alt="" width="192" height="157" decoding="async">`;

  const headerRoot = document.querySelector('[data-shared-header]');
  if (headerRoot && !headerRoot.querySelector('.fuel-site-header')) {
  ...
  if (headerRoot) {
    headerRoot.querySelectorAll('a[href]').forEach((link) => {
      if (isCurrentPage(link.getAttribute('href'))) link.setAttribute('aria-current', 'page');
